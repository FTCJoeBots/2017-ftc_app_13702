...
public class HardwareJoeBot2017
{
    ...
    public int clampPosition=0; // valid Clamp Positions are 1 (closed), 2 (mid open), 3 (full open)
    public int liftHeight=0; // valid lift heights are 0 (based position), 1 (single glyph), 2 (double glyph)
    public int rotatePosition=0; // valid rotate positions are 1 (vertical), 2 (horizontal)


    // Servo and Lift Positions
    static final double ROTATE_SERVO_VERTICAL_POSITION = 0;
    static final double ROTATE_SERVO_HORIZONTAL_POSITION = 0.4;
    static final double CLAMP_SERVO_OPEN_POSITION = 0;
    static final double CLAMP_SERVO_CLOSED_POSITION = 0.75;
    static final double CLAMP_SERVO_MID_POSITION = 0.4;
    static final int LIFT_BASE_POSITION = 0;
    static final int LIFT_SEARCH_ONE_POSITION = 1800;
    static final int LIFT_SEARCH_TWO_POSITION = 3400;
    static final double LIFT_MOTOR_POWER = 0.5;
    ...
    public void clampToggle() {
        ...
        // clampToggle will toggle between the three clamp positions, from open to closed, to mid

        switch(clampPosition){

            case 1:
                // Clamp is closed; move to mid-open position
                clampMid();

            case 2:
                // Clamp is in Mid Position; move to full open;
                clampOpen();

            case 3:
                // Clamp is fully open; Move to closed position:
                clampClose();
    ...
    public void clampClose() {
        ...
        // Set the clamp to closed position
        clampServo.setPosition(CLAMP_SERVO_CLOSED_POSITION);
        clampPosition = 1;
    ...
    public void clampOpen() {
        ...
        // Set the clamp to Open Position
        clampServo.setPosition(CLAMP_SERVO_OPEN_POSITION);
        clampPosition = 3;
    ...
    public void clampMid() {
        ...
        // Set the clamp to Mid Open Position
        clampServo.setPosition(CLAMP_SERVO_MID_POSITION);
        clampPosition = 2;

    }

    public void liftPositionToggle() {

        // Toggle lift between Base Position, search 1 and Search 2;
        // If clamp is horizontal, skip base position

        switch(liftHeight){

            case 0:
                // lift is in Base Position. Move it to Search 1
                liftSearchOnePosition();

            case 1:
                // Lift is in Search 1 Position. Move it to Search 2
                liftSearchTwoPosition();

            case 2:
                // Check to see if clamp is horizontal. If it is, move to Search 1, otherwise move to base
                if(rotatePosition == 1){
                    // Clamp is Vertical. Move to Base Position
                    liftBasePosition();
                } else {
                    // lift must be horizontal. Move to search 1 position
                    liftSearchOnePosition();
                }
    ...
    public void liftBasePosition() {

        // Set the lift to base position

        // First, set lift motor to run to position
    ...
    public void liftSearchOnePosition() {
    ...
    public void liftSearchTwoPosition() {
    ...
    public void rotateClamp() {

        // Toggle between rotatePositions
